 ...
 */
public class SummitStudentImporter {

    private static final Logger LOG = Logger.getLogger(SummitStudentImporter.class);
    ...
    public int importFile(File file) {
        ...
        try {
            ...
            LOG.info("SUMMIT import: " + records.size() + " records from " + file.getName());

            Iterator it = records.iterator();
            while (it.hasNext()) {
                SisStudentRecord record = (SisStudentRecord) it.next();
                if (StringHelper.isEmpty(record.getSisId())) {
                    LOG.warn("SUMMIT import: skipping record with no SIS id");
                    continue;
                }
                Student student = studentDao.findBySisId(record.getSisId());
                if (student == null) {
                    student = new Student();
                    student.setCustomerCode(CustomerContext.SUMMIT);
                    student.setCreatedAt(new Date());
                }
                applyCommonFields(student, record);
                // SUMMIT BRANCH ONLY: home campus, the field the shared Northlake
                // importer silently drops every night on master. See CC-1301.
                student.setHomeCampus(record.getHomeCampus());
        ...
        LOG.info("SUMMIT import finished, " + imported + " records");
        return imported;
    }
